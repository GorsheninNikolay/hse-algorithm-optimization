...
def simplex_method(z_to_max, coefficients, limitations):
    ...
    for i in range(num_vars):
        col = table[:, i]
        if np.sum(np.abs(col - 1) < 1e-8) == 1 and np.sum(np.abs(col) < 1e-8) == num_constraints:
...
if __name__ == '__main__':
    ...
    print(f'Маги={wizards}, Рыцарей={knights}, Драконов={dragons}, целевое значение={target_val}')
